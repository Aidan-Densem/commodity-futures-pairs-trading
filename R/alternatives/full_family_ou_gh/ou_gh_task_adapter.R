...
ou_gh_prepare_selected_formation <- function(
  ...
  list(
    ...
    unique_duration_count = length(unique(round(transitions$delta, 12L))),
    minimum_duration = min(transitions$delta),
    median_duration = stats::median(transitions$delta),
    maximum_duration = max(transitions$delta),
    empirical_transition_duration =
      "all_valid_positive_observed_active_durations"
  )
}
...
ou_gh_build_horizon_pairs <- function(transitions, horizons,
                                      matching_tolerance = NULL) {
  required <- c("segment_id", "x_previous", "x_current", "delta")
  ou_gh_assert(all(required %in% names(transitions)),
    "Full-family horizon input lacks transition states or durations.")
  horizons <- sort(unique(as.numeric(horizons)))
  ou_gh_assert(all(is.finite(horizons) & horizons > 0),
    "Active-time horizons must be finite and positive.")
  output <- list()
  counter <- 0L
  for (segment_id in unique(transitions$segment_id)) {
    one <- transitions[transitions$segment_id == segment_id, , drop = FALSE]
    order_field <- if ("previous_timestamp" %in% names(one)) {
      one$previous_timestamp
    } else if ("previous_global_row" %in% names(one)) {
      one$previous_global_row
    } else seq_len(nrow(one))
    one <- one[order(order_field), , drop = FALSE]
    ou_gh_assert(all(is.finite(one$delta) & one$delta > 0),
      "A full-family segment contains a non-positive observed duration.")
    values <- c(one$x_previous[[1L]], one$x_current)
    active_time <- c(0, cumsum(one$delta))
    timestamps <- if (all(c("previous_timestamp", "current_timestamp") %in%
                           names(one))) {
      c(one$previous_timestamp[[1L]], one$current_timestamp)
    } else rep(as.POSIXct(NA), length(values))
    rows <- if (all(c("previous_global_row", "current_global_row") %in%
                    names(one))) {
      c(one$previous_global_row[[1L]], one$current_global_row)
    } else seq_along(values)
    for (horizon in horizons) {
      tolerance <- matching_tolerance
      if (is.null(tolerance)) {
        tolerance <- 64 * .Machine$double.eps * max(1, abs(horizon))
      }
      for (start in seq_len(length(values) - 1L)) {
        target <- active_time[[start]] + horizon
        hit <- which(seq_along(active_time) > start &
          abs(active_time - target) <= tolerance)
        if (!length(hit)) next
        ou_gh_assert(length(hit) == 1L,
          "An active-time horizon resolved to multiple endpoints.")
        end <- hit[[1L]]
        counter <- counter + 1L
        output[[counter]] <- data.frame(
          segment_id = segment_id,
          horizon = horizon,
          active_horizon = active_time[[end]] - active_time[[start]],
          row_lag = end - start,
          x_previous = values[[start]],
          x_current = values[[end]],
          previous_timestamp = timestamps[[start]],
          current_timestamp = timestamps[[end]],
          previous_global_row = rows[[start]],
          current_global_row = rows[[end]],
          stringsAsFactors = FALSE
        )
      }
    }
